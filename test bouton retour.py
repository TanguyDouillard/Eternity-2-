...
## Affichage classique logo + titre + règles

def affichage_design_classique():
...
def draw_ecran():
    global choix_grille, taille_piece, couleur_fond_ecran ,couleur_fond_grille, couleur_grille, couleur_grille_lignes, couleur_titre, couleur_titre_regles, couleur_texte_regles, couleur_fond_regles, couleur_contour_regles, bouton_retour
    ...
    bouton_retour = pygame.Rect(1200, 30, 200, 60)
    pygame.draw.rect(screen, "white", bouton_retour, border_radius=10)
    texte_retour = font_bouton.render("Retour", True, "black")
...
while running:
    ...
    if scene == 1 :
        ...
        bouton_jouer = pygame.Rect(225, 670, 200, 60)  # Bouton "Jouer"

        run = True
        while run:
            screen.fill(couleur_fond)
            affichage_design_classique()
...
Appuie ici : """
            ...
            # Découper et afficher le texte proprement
            lignes = texte_jouable.split("\n")  # Séparer les lignes avec "\n"
            y = 600  # Décalage du haut

            for ligne in lignes:
                if ligne.strip() == "":  # Vérifier si la ligne est vide pour ajouter un espace vertical
                    y += font_titre.get_height() // 2
                else:
                    texte_render = font_titre.render(ligne, True, (0,0,0))  # Noir
...
Nathan Deltour et Tanguy Douillard"""
            ...
            for ligne in lignes:
                if ligne.strip() == "":  # Vérifier si la ligne est vide pour ajouter un espace vertical
                    ...
                    texte_render = font_texte_fin.render(ligne, True, (0,0,0))  # Noir
